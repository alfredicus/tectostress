...
def calculate_rake_and_movement(normal_vector, shear_stress, azimuth):
    """
    Calculate rake angle and sense of movement - SIMPLE VERSION
...
def calculate_rake_direction(normal_vector, shear_stress, azimuth):
    """
    Calculate the geographic direction from which rake angle is measured
    ...
    Returns:
    - rake_direction: Cardinal direction (N, S, E, W)
    """
    
    if np.linalg.norm(shear_stress) == 0:
        return azimuth_to_cardinal(azimuth)
    ...
    # Calculate opposite azimuth vector
    ...
    # Calculate rake angles for both directions
    cos_rake = np.dot(azimuth_vector, striation)
    cos_rake_opposite = np.dot(opposite_azimuth_vector, striation)
    
    rake_from_azimuth = math.degrees(math.acos(abs(cos_rake)))
    rake_from_opposite = math.degrees(math.acos(abs(cos_rake_opposite)))
    
    # Choose the direction that gives the smaller rake angle
    if rake_from_azimuth <= rake_from_opposite:
        reference_azimuth = azimuth
    else:
        reference_azimuth = opposite_azimuth
    
    # Apply geological convention: if striation points upward (inverse slip),
    # measure from opposite direction
    if striation[2] > 0.1:  # Inverse slip component
        reference_azimuth = (reference_azimuth + 180) % 360
    
    return azimuth_to_cardinal(reference_azimuth)
...
def azimuth_to_cardinal(azimuth):
    ...
    Parameters:
    - azimuth: Azimuth in degrees (0-360, clockwise from North)
...
def validate_fault_plane_vectors(normal_vector, azimuth, dip):
    ...
    # Allow for both orientations (normal can point up or down)
    dot_product = np.dot(computed_normal, normal_vector)
    is_valid = abs(abs(dot_product) - 1.0) < 0.05  # Reasonable tolerance for numerical precision
...
def draw_great_circle(ax, strike, dip, color='gray', alpha=0.5, linewidth=1.5):
    """
    ...
    # Number of points to define the great circle
    n_points = 180
    ...
    # Vector 1: along strike (horizontal)
    v1 = np.array([
        math.sin(strike_rad),   # East
        math.cos(strike_rad),   # North  
        0                       # Horizontal
    ])
    
    # Vector 2: down-dip direction
    dip_azimuth = (strike + 90) % 360  # Perpendicular to strike
    dip_azimuth_rad = math.radians(dip_azimuth)
    
    v2 = np.array([
        math.sin(dip_rad) * math.sin(dip_azimuth_rad),   # East component
        math.sin(dip_rad) * math.cos(dip_azimuth_rad),   # North component
        -math.cos(dip_rad)                               # Down component
    ])
    ...
    for i in range(n_points + 1):
        # Parameter along the great circle
        theta = 2 * math.pi * i / n_points
        
        # Point on the great circle (on unit sphere)
        ...
        # Avoid the south pole (z = -1)
        if point[2] > -0.999:
            denominator = 1 - point[2]
            x_proj = point[0] / denominator
            y_proj = point[1] / denominator
            
            # Only include points within reasonable bounds
            if x_proj**2 + y_proj**2 <= 4:  # Reasonable projection limit
...
def create_stereographic_projection(stress_tensor, fault_data):
    ...
    print(f"   Drawing {len(fault_data)} great circles for fault planes...")
    ...
    for _, row in fault_data.iterrows():
        ...
        # Normal vector (pole to plane) - perpendicular to strike, pointing up
        normal = np.array([
            math.sin(dip_rad) * math.sin(azimuth_rad + math.pi/2),
            math.sin(dip_rad) * math.cos(azimuth_rad + math.pi/2),
            math.cos(dip_rad)  # Upward pointing
        ])
        ...
        if normal[2] > -0.999:
            ...
            ax.plot(x_proj, y_proj, '.', color='darkred', markersize=4, alpha=0.8, zorder=3)
    ...
    ax.set_title('Stereographic Projection (Lower Hemisphere)\nWulff Net with Great Circles\n' +
                'Red=σ1, Green=σ2, Blue=σ3\nBlue lines=Fault planes, Red dots=Poles')
...
def main():
    ...
    for i, normal in enumerate(fault_normals):
        ...
        rake, movement = calculate_rake_and_movement(normal, shear_stress, azimuth)
        
        # Calculate rake direction separately
        rake_direction = calculate_rake_direction(normal, shear_stress, azimuth)
    ...
    print(f"   • Rake angles are acute (0-90°) with direction column (N/S/E/W)")
    print(f"   • Great circles (blue lines) show fault planes on Wulff stereonet")
    print(f"   • Red dots show fault poles (normals to planes)")
    ...
    print(f"   • Rake_Dir column shows cardinal direction from which rake is measured")
